...
{
    public class GetAlarmsByIdQueryHandler : IRequestHandler<GetAlarmsByIdQuery, ResponseBase<AlarmsResponse>>
    {
        private readonly IMediator _mediator;
        private readonly IAlarmsQueryRepository _AlarmsQueryRepository;

        public GetAlarmsByIdQueryHandler(IMediator mediator, IAlarmsQueryRepository AlarmsQueryRepository)
        {
            _mediator = mediator;
            _AlarmsQueryRepository = AlarmsQueryRepository;
        }

        public async Task<ResponseBase<AlarmsResponse>> Handle(GetAlarmsByIdQuery request, CancellationToken cancellationToken)
        {
            ResponseBase<AlarmsResponse> responseBase = new ResponseBase<AlarmsResponse>();

            try
            {
                var Alarms = await _AlarmsQueryRepository.GetAlarmsById(request.Id);
