...
{
    public class GetResultatCommandeCompteursQueryHandler : IRequestHandler<GetResultatCommandeCompteursQuery, ResponseBase<List<ResultatCommandeCompteurResponse>>>
    {
        private readonly IResultatCommandeCompteurQueryRepository _resultatCommandeCompteurQueryRepository;
        private readonly IAuthorizationService _authService;

        public GetResultatCommandeCompteursQueryHandler(
        ...
        public async Task<ResponseBase<List<ResultatCommandeCompteurResponse>>> Handle(GetResultatCommandeCompteursQuery request, CancellationToken cancellationToken)
        {
            ResponseBase<List<ResultatCommandeCompteurResponse>> responseBase = new ResponseBase<List<ResultatCommandeCompteurResponse>>();
            try
            {
                var resultats = await _resultatCommandeCompteurQueryRepository.GetResultatCommandeCompteurs();
                
                if (resultats == null || !resultats.Any())
                {
                    responseBase.IsSuccess = true;
                    responseBase.Message = "Aucun résultat trouvé";
