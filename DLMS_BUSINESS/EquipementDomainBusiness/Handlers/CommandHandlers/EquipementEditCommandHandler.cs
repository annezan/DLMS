...
{
    public class EquipementEditCommandHandler : IRequestHandler<EquipementEditCommand, ResponseBase<EquipementResponse>>
    {
        private readonly IEquipementCommandRepository _EquipementCommandRepository;
        private readonly IMediator _mediator;

        public EquipementEditCommandHandler(IEquipementCommandRepository EquipementCommandRepository, IMediator mediator, IEquipementQueryRepository EquipementQueryRepository)
        {
            _mediator = mediator;
            _EquipementCommandRepository = EquipementCommandRepository;
        }

        public async Task<ResponseBase<EquipementResponse>> Handle(EquipementEditCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var editEquipement = await _EquipementCommandRepository.EditEquipement(EquipementEntity);
                if (editEquipement == null || EquipementEntity == null)
                {
                ...
                responseBase.Data = EquipementMapper.Mapper.Map<EquipementResponse>(editEquipement);
