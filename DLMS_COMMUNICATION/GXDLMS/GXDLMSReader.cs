        ...
        public void ExportMeterCertificates(string logicalName)
        {
            try
            {
                ...
                foreach (GXDLMSCertificateInfo it in ss.Certificates)
                {
                    ...
                    if (!ReadDataBlock(ss.ExportCertificateBySerial(Client, it.SerialNumber, it.Issuer), reply))
                    {
                        throw new GXDLMSException(reply.Error);
                    }
        ...
        public void GetProfileGenerics()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    try
                    {
                        var essai = it as GXDLMSProfileGeneric;
                        //Read last day from Profile Generic.
                        DateTime datestart = DateTime.Parse("01/06/2024 00:00:00");
                        DateTime dateend = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, 0, 0);
                        object[] rows = ReadRowsByRange(it as GXDLMSProfileGeneric, datestart, dateend);
                        //If trace is info.
                        if (Trace > TraceLevel.Warning)
                        {
                            StringBuilder sb = new StringBuilder();
